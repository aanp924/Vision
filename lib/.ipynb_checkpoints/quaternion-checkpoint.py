#Using code from https://github.com/micropython-IMU/micropython-fusion/ 
#Implementing Madgwick Algorithm
#Updated to apply mpu9250 calibration coefficients
...
class Quaternion:
    def __init__(self):
        ...
        self.gyro_bias = np.array([7.5194549560546875, -20.7506103515625, 1.5494155883789062], dtype=np.float64)
        GyroMeasError = radians(40)        #(math.pi * 40) / 180 # # Original code indicates this leads to a 2 sec response time
    ...
    #Pass offset compensated data
    def update(self, accel, gyro, mag, ts = time.perf_counter()):     
        mx, my, mz = mag #(mag[x] - self.magbias[x] for x in range(3)) # Units irrelevant (normalised)
        ...
        deltat = time.perf_counter() - ts #start_time
        q1 += qDot1 * deltat
        q2 += qDot2 * deltat
        q3 += qDot3 * deltat
        q4 += qDot4 * deltat
